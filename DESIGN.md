...
- **Market board (`MarketBoard.astro`):** the `box` beside the cover is a five-row ruled table under a `.module__head` (name Naskh 700 0.95rem, level and move Tajawal tabular, numbers isolated LTR) with a "لوحة الأسواق كاملة · حُدّثت منذ …" foot; the `full` board opens الأسواق: four groups (المؤشرات, العملات, السلع, السندات والأصول الرقمية) two across on a rule, each a table with a Tajawal 0.74rem head on a 1px ink rule and the columns الأداة (name over a note: country or unit), آخر, التغير, %, الوقت (time today, else date), and a foot that says the prices are delayed, from which feed, and when. Stale rows print their numbers in ink-3. `--up` (`#0f5c3c`, dark `#67c096`) and `--down` (`#a3271f`, dark `#e2796d`) are the one functional colour pair on the site; nothing else is red.
