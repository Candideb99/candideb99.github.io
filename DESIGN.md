...
- **Section panels:** ≥1024px each section link in the bar opens, on hover or focus, a paper panel under the bar closed by a 1px ink rule: three ruled columns `3fr | 6fr | 3fr`, heads Tajawal 800 0.78rem green (المواضيع · تحليلات القسم · المزيد), topics and links Naskh 0.98rem ink-2, analysis titles Amiri 700 ink; the hubs' panels list their latest pieces. Below 1024px the bar scrolls and the sections are plain links. An inline script rewrites the date, issue number and every `time[data-relative]` each minute so a static build stays current.
