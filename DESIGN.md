...
typography:
  ...
  furniture-sm:
    fontFamily: "Tajawal, IBM Plex Sans Arabic, Segoe UI, system-ui, sans-serif"
    fontSize: "0.86rem (0.875rem ≤719px), token --fs-sm"
    fontWeight: 500
    lineHeight: 1.5
  furniture-xs:
    fontFamily: "Tajawal, IBM Plex Sans Arabic, Segoe UI, system-ui, sans-serif"
    fontSize: "0.78rem (0.8125rem ≤719px), token --fs-xs"
    fontWeight: 700
    lineHeight: 1.4
rounded:
  none: "0"
...
| Cover story title | Amiri 700 | `clamp(1.7rem, 1.1rem + 1.9vw, 2.5rem)` | 1.28 |
...
**The Isolated Figure Rule.** Digits are Western and tabular (`.num`, `time`, `.ui`, figure values); `isolateNumbers()` wraps each figure with its sign and % in `<bdi dir="ltr">` and turns a leading hyphen into U+2212; a figure of several groups (a fiscal year 2026/27, a date 2026-09-19, a time 14:30, a range 2020–2025) is one run, since split into runs the right-to-left line printed it backwards («27/2026», until 2026-09-22). `lib/bidi.ts` and `lib/rehype-isolate-numbers.mjs` carry the same pattern. Dates read "الأربعاء 9 سبتمبر 2026", short "7 سبتمبر", time "22:25 بتوقيت الرياض"; relative times "الآن / قبل 3 ساعات / أمس / قبل يومين / قبل 5 أيام", then a date; the issue number is days since 2026-09-07 + 1.
...
- **Article**: head max 52rem (title, dek, meta on a hairline carrying both calendars, «نُشر الخميس 24 سبتمبر 2026 م، 13 ربيع الآخر 1448 هـ · 16:00 بتوقيت القاهرة», as الشرق الأوسط prints them (`formatHijri()`, 2026-09-24), share row) → figure at story width, 2/1 crop (3/2 ≤719px) → `1fr | 17rem`: main (lede, inline facts ≤1023px, لماذا يهمّ, chart, prose, table, tags, sources) and a rail with a 1px start rule (side facts, اقرأ أيضاً); ≤1023px the rail stacks beneath. لماذا يهمّ follows the lede and the figures at the text's own size (what happened, the numbers, why it matters, then the full story); until 2026-09-22 it came after the whole text, a step smaller, four screens down on a phone. ≤719px the lede is set at the text's size (at a size above it, justified in 343px, its word gaps reached nearly four spaces).
...
### Masthead and brand
- **Top line (`Masthead.astro`):** Tajawal 500 `--fs-sm` ink-2 on a hairline: weekday date · the Hijri date by the Umm al-Qura calendar in Western digits («12 ربيع الآخر 1448 هـ», as Asharq Al-Awsat prints it) at the start; no issue number (2026-09-23: an online news website, not a paper). At the end the standing tools in Tajawal 700 ink-2 (الأحدث، لوحة الأسواق، الأجندة) and a 16px magnifier + "بحث" (ink, green on hover), each a 44px target inside the line's own height (padding with a matching negative margin); the whole group is hidden ≤719px, where the search and the menu live in the section bar.
- **Phone menu (≤719px):** a three-bar icon (aria-label القائمة) at the end of the section bar after a magnifier, both 40px targets in paper on green, a native `<details>` (it works without script; a small script closes it on Escape, returning focus, and on a tap outside). Its panel hangs under the bar across the column, paper on hairline sides closed by the 2px ink rule the section panels use: الأقسام (الرئيسية and the eight sections, two columns, the current one green) and الأدوات والبيانات (الأحدث، لوحة الأسواق، الأجندة الاقتصادية، الملفات والمناطق), Naskh 700 1.02rem rows a finger's height apart on hairlines. It exists because a phone's section bar shows five of its nine names and the tools line is hidden there.
- **Section panels:** ≥1024px with a hovering pointer (`hover: hover`), each section link in the bar opens, on hover only, a paper box the size of its content and never wider than the bar (`max-width: 100%` of the wrap), hung from the bar's start edge (hairline sides, 2px ink bottom; the open link turns paper with green text): columns of at least 11rem ruled between, heads Tajawal 800 0.76rem green (المواضيع · تحليلات القسم · المزيد), the standing sub-topics in Naskh 0.96rem ink-2 in two columns when there are more than five (they may wrap when the bar is narrow), analysis titles Amiri 700 ink in a column capped at 22rem; the hubs' panels list their kinds or desks and latest pieces. A panel opens after the pointer has rested on its section for 150ms (`visibility` with a transition delay), so a pointer crossing the bar no longer flashes panels. Keyboard focus never opens a panel (one held open by focus used to sit over the section under the pointer); below 1024px and on touch screens the bar scrolls and the sections are plain links, and the same taxonomy heads every section page. The sub-topics are a curated taxonomy (`topics.json`), never mined tags. An inline script rewrites the date, the Hijri date and every `time[data-relative]` each minute so a static build stays current.
...
- **Section bar:** the one green field: links Naskh 700 `#fcfcf9` on `#0f5c3c`, padding 0.62rem 1.1rem 0.5rem, centred (start-aligned and scrollable ≤719px, scrollbar hidden); hover white 12% tint; `aria-current="page"` a 3px `#fcfcf9` underline plus 8% tint; focus outline `#fcfcf9` inset 3px. Order: الرئيسية, then the six sections. **Pinned** (2026-09-23, as the bars of Al Jazeera, Asharq Business and the BBC are): the bar is a sibling of the header (`position: sticky; top: 0; z-index: 40`) and stays at the top of the screen for the whole page; once the header has scrolled away (an IntersectionObserver marks it `is-stuck`) a 30px paper-lettered logo (`/logo-dark.svg`, 26px ≤719px) takes the place of الرئيسية at its start and a magnifier appears at its end, both fading in over 0.18s unless motion is reduced. `html { scroll-padding-top: 3.75rem }` keeps anchor jumps clear of it.
- **Market strip (`MarketStrip.astro`):** on every page between the section bar and the ticker, paper ground under a hairline, 2.1rem tall: a green "الأسواق" label (Tajawal 800 0.78rem, a link to the board) then eight instruments on hairline dividers, each its short name (Tajawal 700 ink) and its move on the day (Tajawal 700, `--up` green or `--down` red with a small ▲/▼), the level in the tooltip, "حُدّث منذ …" at the end (hidden ≤719px). Overflow scrolls with a hidden scrollbar inside the list, which is `position: relative` so the hidden ارتفاع/انخفاض words stay inside it (without it they widened every phone page by 505px and phones opened the paper on blank margin, until 2026-09-22). Items carry 0.4rem of block padding, a finger-sized target. Sized so the eight fit one line at 1240px. A move that rounds to zero prints «0.00», never «−0.00».
...
- **Ticker (أيضاً على خازندار):** `--paper-2` strip 2.4rem tall under a hairline, green "أيضاً على خازندار" label (Tajawal 800 `--fs-xs`), five headlines Naskh 700 0.95rem each preceded by its section in green Tajawal, moving by whole pixels (60px/s) as the owner asked; it holds under the pointer, under keyboard focus and under a finger (a touch stops it, a swipe scrolls it by hand, it moves on three seconds after the finger lifts, from wherever it was left); under reduced motion or without script it stands still and scrolls by hand. **آخر الأخبار (2026-09-24, Task 4 after reading الشرق الأوسط beside Khazendar):** the same component under the market strip on every other page (the base layout builds it; not on the front page, which runs its own, nor on /latest/, which is the same list at full length, nor on the 404): label «آخر الأخبار», the eight newest news stories without the page's own, each preceded by its age in green Tajawal (`<time data-relative>`, kept current between builds by the layout's script), since there the ages are hours, not the days that read badly under the old الأحدث.
...
- **Variants (`StoryCard.astro`):** `lead` (16/9 photo, 200-char summary Naskh 1.05rem/1.7), `card` (16/9 photo above, optional 120-char summary 0.98rem), `list` (row, 5.5rem square photo at the start), `text` (no photo). Meta: section link in green (on the hubs' pages and bands the story's kind instead: the analysis desk, شرح مبسّط, قراءة في ورقة بحثية) + relative time; `showPhoto=false` drops the picture entirely.
...
- **Pill (tags):** Tajawal 500 0.86rem ink-2, 1px `--rule` border; hover/focus turns border and text ink. **Share (`Share.astro`):** "شارك" label, then واتساب / إكس / تيليغرام / فيسبوك links and a "نسخ الرابط" button (reads "تم النسخ" for 2s, announced through a polite live region): Tajawal 700 `--fs-xs` ink-2 on paper, 1px `--rule` border, padding 0.55rem 0.7rem 0.45rem, the chips wrapping beside the label; green border and text on hover. On a touch screen with a share sheet (`navigator.share` and a coarse pointer) the row is واتساب and «تطبيقات أخرى», the device's own sheet: one line instead of five chips over two.
...
- **Kicker** (`.kicker`, global): the story's topic (its most-shared tag that names no place: never a country or a desk, `PLACE_TAGS`, 2026-09-24) in Tajawal 700 0.78rem green above the headline, linking to the topic's file, on the cover, the secondaries, the block features, the section-page lead and the article page; never on thumbnails or briefs. Relative times read منذ ساعة / منذ 3 ساعات / منذ يوم / منذ يومين; the article meta line reads نُشر with date and Riyadh time.
...
- **Latest (`latest/[...page].astro`):** the ruled list of everything, newest first, under a dated head for each day (Tajawal 700 `--fs-sm` on a 1px ink rule), so a reader sees where yesterday ends.
- **Search (`search.astro`):** input Naskh 1.3rem on paper, no border but a 2px ink bottom rule that turns green and 3px thick on focus (outline suppressed); the count agrees with its number (نتيجة واحدة، نتيجتان، 3 نتائج، 67 نتيجة) and says when only the first twenty are shown; results max 46rem on hairlines, titles Amiri 1.2rem, excerpts Naskh 1rem ink-2, `mark` green bold without background, count and hints Tajawal ink-3 in an `aria-live="polite"` region.
- **Article meta** (`[slug].astro` `.article__meta`): two lines on purpose, the trail (section › sub-topic) with the byline, then the date in both calendars, the hour with its zone and the reading time, the last two each kept whole (`.article__whole`) so a phone never leaves «بتوقيت القاهرة» or «دقيقتا قراءة» alone on a line (2026-09-24).
- **Excerpts** (`excerpt()` in `src/lib/format.ts`): cut at the last clause mark («،» «؛» «.» «:») in the second half of the room, else at a word; never on a joining word («في»، «و»، «كلما») and never inside an open «…» (2026-09-24).
- **Sources**: an English headline takes the Latin face (Tajawal, 0.94rem) at the Arabic titles' size; the Arabic faces fell back to Amiri's larger Latin.
