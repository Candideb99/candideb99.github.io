...
- **Section panels:** ≥1024px each section link in the bar opens, on hover or focus, a paper box the size of its content hung under its own section link (from the link's end edge for the far half of the bar; hairline sides, 2px ink bottom; the open link turns paper with green text; the hovered item is raised above any panel kept open by focus): columns of at least 11rem ruled between, heads Tajawal 800 0.76rem green (المواضيع · تحليلات القسم · المزيد), the standing sub-topics in Naskh 0.96rem ink-2 in two columns when there are more than five, analysis titles Amiri 700 ink in a column capped at 22rem; the hubs' panels list their kinds or desks and latest pieces. Below 1024px the bar scrolls and the sections are plain links. The sub-topics are a curated taxonomy (`topics.json`), never mined tags. An inline script rewrites the date, issue number and every `time[data-relative]` each minute so a static build stays current.
