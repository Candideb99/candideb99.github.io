...
### Masthead and brand
- **Top line (`Masthead.astro`):** Tajawal 500 0.84rem ink-2 on a hairline: weekday date · "العدد N" at the start; at the end the standing tools in Tajawal 700 ink-2 (الأحدث، لوحة الأسواق، الأجندة; hidden ≤719px) and a 16px magnifier + "بحث" (ink, green on hover).
- **Section panels:** ≥1024px with a hovering pointer (`hover: hover`), each section link in the bar opens, on hover only, a paper box the size of its content and never wider than the bar (`max-width: 100%` of the wrap), hung from the bar's start edge (hairline sides, 2px ink bottom; the open link turns paper with green text): columns of at least 11rem ruled between, heads Tajawal 800 0.76rem green (المواضيع · تحليلات القسم · المزيد), the standing sub-topics in Naskh 0.96rem ink-2 in two columns when there are more than five (they may wrap when the bar is narrow), analysis titles Amiri 700 ink in a column capped at 22rem; the hubs' panels list their kinds or desks and latest pieces. Keyboard focus never opens a panel (one held open by focus used to sit over the section under the pointer); below 1024px and on touch screens the bar scrolls and the sections are plain links, and the same taxonomy heads every section page. The sub-topics are a curated taxonomy (`topics.json`), never mined tags. An inline script rewrites the date, issue number and every `time[data-relative]` each minute so a static build stays current.
...
- Reduced motion collapses transitions to 0.01ms and drops the cover's fade; the cover still turns (the owner's decision), pausing under the pointer or keyboard focus and resting for a minute after a dot click; its dots are a `tablist` over `tabpanel` slides. The market moves carry a visually-hidden ارتفاع/انخفاض word; the ticker and market strip are labelled regions.
