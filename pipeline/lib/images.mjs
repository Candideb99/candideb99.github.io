...
const PLACE_WORDS = new Set(Object.entries(PHOTO_PLACES).flatMap(([country, names]) => [country, ...names]).flatMap((n) => String(n).toLowerCase().split(/\s+/)));
/**
 * `names`: a phrase that is a name alone («Central Bank of Egypt») is about its own words less its place (central,
 * bank); without that, every file matching "Egypt" passed, and the judge was shown six rooms of the Luxor Museum
 * (2026-09-24). The last pass keeps the lower-case words only: it must never search a named building.
 */
function subjectWords(query, { names = false, place = [] } = {}) {
...
function relevance(image, words) {
  ...
  const hit = words.filter((w) => {
    ...
    // A long stem, or a whole word of five letters or more, also counts inside a compound («Gigafactory» is a
    // factory, a «HyperMarket» a market: a Cairo supermarket's shelves were dropped as off-subject, 2026-09-24); a
    // short stem only as a word's start («oil» is not in «soil»).
    return said.some((t) => t.startsWith(stem) || (stem.length >= 5 && t.includes(stem)) || (w.length >= 5 && t.includes(w)));
  }).length;
...
const CONCEPT_RULE = `THIS PIECE EXPLAINS OR ANALYSES (it is not a news report): picture its subject at work in the economy, the way the agencies picture it: traders at their screens or a trading floor for bonds, rates and markets; shoppers and price tags at an everyday popular food market (vegetable and fruit stalls, bread, a butcher) for prices and inflation, never a tourist bazaar; a container port for trade; a factory line for industry; a tanker, pumpjacks or a refinery for oil. A government's or an institution's building front does not picture a concept: refuse it unless the piece is about that institution's own decision.`;
